...
const parseCode = (raw: FormDataEntryValue | null, allowed: number[]): number | null => {
...
export const actions: Actions = {
    donate: async ({ request }) => {
        ...
        if (!email || email.length > MAX_EMAIL_LENGTH || hasHeaderControlChars(email) || !isValidEmail(email)) {
        ...
        try {
            ...
            try {
                await db.delete(donationsTable).where(eq(donationsTable.id, insertedRowId));
