...
test.describe('donation smoke', () => {
    ...
    test('POST /donation?/donate records row, returns UUID, sends confirmation email', async ({
        ...
        expect(status, `unexpected status ${status}; body: ${responseText}`).toBe(200);
        ...
        expect(responseText).not.toMatch(/"message":"(invalid_|server_error|db_error|send_failed)"/);

        if (MAILHOG_URL) {
            const inboxUuid = await pollMailhogForUuid(request, MAILHOG_URL, TARGET_EMAIL);
            expect(inboxUuid).toBe(uuid);
        }
    });

    test('rejects malformed email with 400 invalid_email', async ({ request }) => {
        const form = new URLSearchParams({
            gender: '1',
            age: '2',
            lang: '1',
